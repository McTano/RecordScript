...
class SParsers extends Parsers {
  type Elem = Token
  def numParser(tokens: Input): ParseResult[Num] = {
    tokens.first match {

      case (n: Num) => Success(n, tokens.rest)

    }
  }
  def apply(tokens: Input): ParseResult[Expression] = {
    numParser(tokens)
  }
//   class OpCombinator extends Parser[Token] {}
//   class ExpressionCombinator extends Parser[Token] {}
...
object Solution {
  ...
  def parse(str: String): Expression = {
    new SParsers().apply(new TokenReader(tokenize(str))) match {
      case Success(expr) => {
        expr
      }
    }
  ...
  def parse(tokens: List[Token]): Expression = {
    parseHead(tokens) match {
      ...
      case (t, tokens) =>
        throw new RuntimeException(
          s"expected a single expression, got ${tokens}"
        )
  ...
  def parseHead(tokens: List[Token]): (Expression, List[Token]) = {
    tokens match {
      ...
      case CloseParen :: rest =>
        throw new RuntimeException(
          "Unexpected close paren encountered. Expecting Expression ${tokens}"
        )
      case _ =>
        throw new RuntimeException(s"Unrecognized pattern of tokens: ${tokens}")
    }
  }
  ...
  def parseList(tokens: List[Token]): (Expression, List[Token]) = {
    tokens match {
      ...
      case _ =>
        throw new RuntimeException(
          s"List Expression does not start with a keyword: ${tokens}"
        )
    }
  }
  ...
  def parseTwo(tokens: List[Token]): (Expression, Expression, List[Token]) = {
    parseHead(tokens) match {
      case (e1, tail1) =>
        parseHead(tail1) match {
          case (e2, tail2) =>
            tail2 match {
              case CloseParen :: tail3 => (e1, e2, tail3)
              case _ =>
                throw new RuntimeException(
                  s"Expected two subexpressions followed by a parenthesis, got ${tokens}"
                )
            }
        }
    }
  ...
  def parseBindings(
      tokens: List[Token]
  ): (List[(Var, Expression)], Expression, List[Token]) = {
    parseHead(tokens) match {
      case (targetExpression, CloseParen :: rest) =>
        (Nil, targetExpression, rest)
      case (v: Var, rest1) =>
        parseHead(rest1) match {
          case (e: Expression, rest2) =>
            parseBindings(rest2) match {
...
// println(Solution.evaluate("(let var 78 b 77 (let c 33 (add c (mult var 66))))"))
